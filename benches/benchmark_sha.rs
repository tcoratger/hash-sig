...
pub fn benchmark_signature_scheme<S: SignatureScheme>(c: &mut Criterion, description: &str) {
    ...
    group.bench_function("- sign", |b| {
        b.iter(|| {
            ...
            // Benchmark signing
            let _ =
                S::sign(black_box(&mut rng), black_box(&sk), black_box(epoch), black_box(&message));
        });
    });
    ...
    group.bench_function("- verify", |b| {
        b.iter(|| {
            ...
            let _ = S::verify(black_box(&pk), *epoch, black_box(message), black_box(signature));
        });
    });
...
pub fn bench_function_sha(c: &mut Criterion) {
